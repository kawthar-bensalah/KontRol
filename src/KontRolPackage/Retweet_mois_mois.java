...
public class Retweet_mois_mois extends JFrame {
    static int nb;
	private static final long serialVersionUID = 1L;
	public Retweet_mois_mois() {        
        initUI();
    }
    private void initUI() {
    ...
    private CategoryDataset createDataset(int nb) {
		...
		while(nbr<nb)
		{ 
		type[nbr] = format1.format(c.getTime());
        ...
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
         nbr=0; int nbr2=nb-1;
        while(nbr2>-1)
        {  dataset.setValue(tab[nbr2], "Nombre de retweets", type[nbr2]);  nbr2--;}     
        return dataset;
    }
    private JFreeChart createChart(CategoryDataset dataset) {
        JFreeChart barChart = ChartFactory.createBarChart("Nombre de retweets pendant le(s) "+nb+" dernier(s) mois ","Valeurs","Nombre de retweets",dataset,PlotOrientation.VERTICAL,
            false, true, false);
