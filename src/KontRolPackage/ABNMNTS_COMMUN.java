...
public class ABNMNTS_COMMUN extends javax.swing.JFrame {

    public ABNMNTS_COMMUN() {
        initComponents();
    }
    ...
    public void ExtraireAbonnementsEnCommuns(String user) throws TwitterException
    {
        ...
        ids = cnx.getFriendsIDs(cnx.getScreenName(),-1);
        ...
        {
            liste.setText("<Listes d'abonnements vides !>");
        }
        else
        {
            ...
            if(enfant.isEmpty())
                ...
                liste.setText("<L'utilisateur: "+user+" n'as pas d'abonnements>");

             else
             {
                 if(parent.isEmpty())
                     liste.setText("<L'utilisateur: "+cnx.getScreenName()+" n'as pas d'abonnements>");
            ...
            else
             {
                ...
                //pour la configuration
                  ...
                  if(ABONNEMENTS_CONFIG.ok)
                  {
                      int limite =ABONNEMENTS_CONFIG.nombre;

                      if(enfant.size()<limite-1 && parent.size()<limite-1)
                          liste.setText("Extraction impossible: \nLes listes d'abonnements des utilisateurs contiennent moins de "+limite+" abonnements.");
                      else
                      {
                      if(enfant.size()<limite-1)

                          liste.setText("Extraction impossible: \nLa liste d'abonnements de "+user+" contient moins de "+limite+" abonnements.");
                      else
                      {
                          if( parent.size()<limite-1)
                              liste.setText("Extraction impossible: \nLa liste d'abonnements de "+cnx.getScreenName()+" contient moins de "+limite+" abonnements.");
                      ...
                      else
                      {
                        //comparer les 'limite' abonnements
                         while(i<limite-1)
                          ...
                          if(compteur == 0 )
                             liste.setText("<Pas d'abonnements en commun dans les "+limite+"derniers abonnements>");
                      }
                  }
                  ...
                  else
                  {
                        ...
                        if(compteur == 0 )
                           liste.setText("<Pas d'abonnements en commun>"); 
                  }
    ...
    private void btnEtatAbnCommun_ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEtatAbnCommun_ActionPerformed
        ...
        try {
            ...
            ExtraireAbonnementsEnCommuns(LOGIN_CIBLE.userCible);
        } catch (TwitterException ex) {
            Logger.getLogger(ABONNES_COMMUNS.class.getName()).log(Level.SEVERE, null, ex);
        }
