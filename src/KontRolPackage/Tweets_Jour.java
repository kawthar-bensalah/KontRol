...
public class Tweets_Jour extends ApplicationFrame {
	private static final long serialVersionUID = 1L;
	static int nb;
    public Tweets_Jour(final String title) throws TwitterException {
        super(title);
...
while (true) {
  ...
  //les retweets
      nbr=0; int nbr2=nb-1;
        while(nbr<nb && nbr2>-1)
        {  dataset.setValue(tab[nbr], "Nombre de tweets", type[nbr2]); nbr++; nbr2--;}     
        return dataset;
    }
    ...
    private JFreeChart createChart(final CategoryDataset dataset) {      
        final JFreeChart chart = ChartFactory.createLineChart(
            "Nombre de tweets pendant le(s) "+nb+" dernier(s) jour(s) ",       
            "Jours",                   
            "Valeurs",                   
            dataset,                   
